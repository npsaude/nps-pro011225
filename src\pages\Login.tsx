...
import { Mail, Lock, ArrowRightCircle, Stethoscope, ArrowLeft } from "lucide-react";
...
const LOGO_URL = "https://pokyribuibmbeorrcsgk.supabase.co/storage/v1/object/sign/NPS-pro/site/logo-conmagic-favicon.png?token=eyJraWQiOiJzdG9yYWdlLXVybC1zaWduaW5nLWtleV9kZDc4YzM5NC1hMTFlLTQ3MTEtYTVmNi1lMjU4ZGU4MGRiYzgiLCJhbGciOiJIUzI1NiJ9.eyJ1cmwiOiJOUFMtcHJvL3NpdGUvbG9nby1jb25tYWdpYy1mYXZpY29uLnBuZyIsImlhdCI6MTc2OTE4NTA3OSwiZXhwIjoxNzcwMDQ5MDc5fQ.jSiOZo0BFqGup9t3gAzfohZbOwBKpvHRUCGrb_1Fbeg";
...
const Login = () => {
    ...
    const videoId = useMemo(() => {
        return (extractYouTubeId(youtubeSetting) ?? extractYouTubeId(FALLBACK_YOUTUBE_VIDEO_ID));
    }, [youtubeSetting]);
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : "Não foi possível fazer login. Verifique seus dados.";
            showError(message);
        } finally {
            setIsLoading(false);
        }
    ...
    const handleCreateAccount = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : "Não foi possível criar o usuário. Verifique os dados.";
            showError(message);
        } finally {
    ...
    const handleAdminShortcutLogin = async () => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : "Não foi possível fazer login rápido. Verifique o usuário de teste.";
            showError(message);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div
            className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-background text-foreground">
            <SubscriptionExpiredDialog open={subscriptionExpiredOpen} onOpenChange={setSubscriptionExpiredOpen} />
            {}
            {youtubeEmbedUrl ? (<div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
                <iframe
            ...
            <div
                className="pointer-events-none absolute inset-0 z-10 bg-[radial-gradient(circle_at_0%_0%,rgba(254,230,122,0.10)_0,rgba(0,0,0,0.25)_55%),radial-gradient(circle_at_100%_100%,rgba(212,160,23,0.06)_0,rgba(0,0,0,0.25)_55%)]" />
            <div
                className="relative z-20 flex w-full max-w-md flex-col items-center px-4 py-8 sm:py-10">
                <div className="mb-6 flex flex-col items-center">
                    <div
                        className="mb-3 flex h-14 w-14 items-center justify-center rounded-3xl bg-card shadow-[0_12px_30px_rgba(0,0,0,0.55)] ring-1 ring-border">
                        <img
                            src={LOGO_URL}
                            alt="Logo CONMEDIC"
                ...
                <div
                    className="w-full rounded-[32px] bg-card px-6 py-7 text-card-foreground shadow-[0_24px_70px_rgba(0,0,0,0.45)] ring-1 ring-border backdrop-blur-md sm:px-8 sm:py-8">
                    {showRegister ? (<div className="mb-5 flex flex-col gap-3">
                        <button
                            type="button"
                    ...
                    {!showRegister ? (<form className="space-y-4" onSubmit={handleSubmit}>
                        <div className="space-y-1.5">
                            ...
                            <div
                                ...
                                <span
                                    className="flex h-11 w-11 items-center justify-center rounded-l-xl border-r border-border text-muted-foreground">
                        ...
                        <p className="pt-1 text-center text-xs text-muted-foreground">Não tem uma conta?{" "}
                            <button
                                type="button"
