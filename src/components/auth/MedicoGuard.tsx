import { useEffect } from "react";
import { useNavigate } from "react-router-dom";

import { showError } from "@/utils/toast";
import { useSystemUser } from "@/hooks/use-system-user";

type Props = {
  children: React.ReactNode;
  redirectTo?: string;
};

/**
 * Guarda das rotas /medico/*. Exige um usuário autenticado com papel
 * MEDICO, ADMIN ou SUPER_ADMIN (admins mantêm acesso para suporte).
 * Bloqueia não autenticados e papéis inesperados (corrige o achado F-4).
 */
export default function MedicoGuard({ children, redirectTo = "/login" }: Props) {
  const navigate = useNavigate();
  const { systemUser, loading } = useSystemUser();

  const role = String(systemUser?.regra ?? "").trim().toUpperCase();
  const isAllowed = role === "MEDICO" || role === "ADMIN" || role === "SUPER_ADMIN";

  useEffect(() => {
    if (loading) return;

    if (!systemUser) {
      showError("Faça login novamente para acessar esta área.");
      navigate("/login");
      return;
    }

    if (!isAllowed) {
      showError("Acesso restrito.");
      navigate(redirectTo);
    }
  }, [loading, systemUser, isAllowed, navigate, redirectTo]);

  if (loading) return null;

  if (!isAllowed) return null;

  return <>{children}</>;
}
