    ...
    try {
      ...
      if (consistencyCheckEnabled) {
        const { data: solData } = await supabase
          ...
          .select("nome_beneficiario, hora_inicial, profissional_nome, profissional_numero_conselho")
          .eq("faturamento_id" as never, ensuredFaturamentoId)
          .maybeSingle();

        if (solData) {
          // Buscar códigos de procedimentos da solicitação
          const { data: solItens } = await supabase
            .from("guia_solicitacao_itens")
            .select("codigo_procedimento")
            .eq("guia_id" as never, (solData as any).id);
          ...
          const results = checkAposSolicitacao(
            {
              ...
              hora_inicial: (solData as any).hora_inicial,
        ...
        if (fatData) {
          ...
          if (solicitacaoEnviada) {
            const { data: solRow } = await supabase
              ...
              .maybeSingle();
            if (solRow) {
              solDataForAut = solRow as typeof solDataForAut;
              const { data: solItens } = await supabase
                .from("guia_solicitacao_itens")
                .select("codigo_procedimento")
                .eq("guia_id" as never, (solRow as any).id);
              solProcCodesForAut = (solItens ?? [])
                .map((it: any) => it.codigo_procedimento)
                .filter(Boolean);
            }
          }
        ...
        if (solicitacaoEnviada) {
          ...
          if (sol) {
            ...
            const { data: solItens } = await supabase
              .from("guia_solicitacao_itens")
              .select("codigo_procedimento")
              .eq("guia_id" as never, (sol as any).id);
            solProcCodesForCheck = (solItens ?? [])
              .map((it: any) => it.codigo_procedimento)
              .filter(Boolean);
          }
        }
    ...
    <div className="min-h-screen bg-[#0b0b0b] text-[#F5F5F5] relative overflow-hidden">
        ...
        <main className="flex flex-1 flex-col items-center justify-start">
          ...
          {view === "pergunta_honorarios" && (
            <div className="mt-2 flex w-full max-w-md flex-col items-center text-center">
              <div className="mb-6">
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-[#FFD700] to-[#D4A017] shadow-[0_0_30px_rgba(212,160,23,0.4)]">
                ...
                <p className="mt-2 text-sm text-[#9CA3AF]">
                  Deseja gerar a guia de honorários automaticamente?
                </p>
              </div>
          ...
          {/* TELA 5.1 - GERANDO HONORÁRIOS */}
          {view === "gerando_honorarios" && (
            <div className="mt-2 flex w-full max-w-md flex-col items-center text-center">
              <div className="mb-6">
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-[#FFD700] to-[#D4A017] shadow-[0_0_30px_rgba(212,160,23,0.4)] animate-pulse">
                ...
                <p className="mt-2 text-sm text-[#9CA3AF]">
                  Aguarde enquanto preenchemos a guia automaticamente.
                </p>
              </div>
            </div>
          )}
          ...
          {view === "sem_modelo" && (
            <div className="mt-2 flex w-full max-w-md flex-col items-center text-center">
              <div className="mb-6">
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-black/60 border border-[#D4A017]/20">
                ...
                <p className="mt-2 text-sm text-[#9CA3AF]">
                  Não há modelo de guia de honorários cadastrado para esta instituição.
                </p>
              </div>
            ...
            <div className="mt-2 flex w-full max-w-md flex-col">
              <div className="mb-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    className="rounded-lg bg-black/60 px-2 py-1 text-[10px] text-[#9CA3AF] border border-[#D4A017]/15 hover:border-[#D4A017]/30"
                    onClick={() => setGuiaZoom((z) => Math.max(0.3, z - 0.1))}
                  >
                    −
                  </button>
                  ...
                  <button
                    type="button"
                    className="rounded-lg bg-black/60 px-2 py-1 text-[10px] text-[#9CA3AF] border border-[#D4A017]/15 hover:border-[#D4A017]/30"
                    onClick={() => setGuiaZoom((z) => Math.min(1.5, z + 0.1))}
                  >
                    +
                  </button>
                </div>
              ...
              <div className="mb-4 overflow-auto rounded-xl border border-[#D4A017]/15 bg-white" style={{ maxHeight: "60vh" }}>
                <div
                  style={{ transform: `scale(${guiaZoom})`, transformOrigin: "top left", width: `${100 / guiaZoom}%` }}
                  dangerouslySetInnerHTML={{ __html: htmlGuiaPreenchida }}
                />
              </div>

              {isGeneratingPdf && (
                <div className="mb-3 flex items-center justify-center gap-2 text-xs text-[#D4A017]">
                  <Loader2 className="h-4 w-4 animate-spin" />
                  <span>Gerando PDF...</span>
                </div>
              )}

              {pdfGerado && pdfBlobUrl && (
                <Button
                  type="button"
                  variant="outline"
                  className="mb-3 h-9 w-full rounded-lg border-[#D4A017]/30 bg-black/40 text-xs font-semibold text-[#D4A017] hover:bg-[#D4A017]/10 hover:text-[#FFD700]"
          ...
          {view === "success" && (
            <div className="flex w-full flex-1 items-center justify-center">
              <div className="flex w-full max-w-sm flex-col items-center text-center">
                <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-[#FFD700] to-[#D4A017] shadow-[0_0_40px_rgba(212,160,23,0.5)]">
      ...
      {showAnalyzingScreen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm">
          <div className="flex w-full max-w-sm flex-col items-center text-center px-6">
            <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-[#FFD700] to-[#D4A017] shadow-[0_0_40px_rgba(212,160,23,0.5)] animate-pulse">
            ...
            <p className="mt-2 text-sm text-[#9CA3AF]">
              {analyzingDocType === "solicitacao"
                ? "Processando guia de solicitação"
                : analyzingDocType === "guia"
                  ? "Processando guia de autorização"
                  : "Processando descrição cirúrgica"}
            </p>
            <div className="mt-6 w-full">
              <Progress value={analyzingProgress} className="h-2" />
              <p className="mt-2 text-xs text-[#6B7280]">{analyzingProgress}%</p>
            </div>
          </div>
        </div>
      )}
