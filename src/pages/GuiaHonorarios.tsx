...
const GuiaHonorariosPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, profissional_nome, profissional_crm, paciente_nome, convenio, data, hora, proc_cir_1_descricao, created_at"
      )
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-400 hover:bg-slate-50">
                      <TableHead className="py-3 pl-4">Profissional</TableHead>
